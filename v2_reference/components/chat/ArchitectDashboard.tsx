...
export function ArchitectDashboard({ onExit, messages, isStreaming, onSend }: ArchitectDashboardProps) {
  ...
  const { data: recentInsights } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`/api/user/insights?limit=5`); // We'll need to update this API
      return res.json();
    },
  ...
  return (
    ...
    >
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-6 bg-stone-900/50 rounded-2xl border border-stone-800/30">
                      ...
                      <div className="text-3xl font-black text-white">{health?.activeSessions24h * 150} <span className="text-xs text-stone-600 uppercase tracking-widest">Tokens</span></div>
                    </div>
      ...
      <div className="p-4 text-center border-t border-stone-900 bg-stone-950/50">
        <p className="text-[7px] text-stone-700 uppercase tracking-[0.5em]">Sovereign Control Interface • v2.1.0-fortress</p>
      </div>
    </motion.div>
  );
}
