...
export function StreamingChat({ 
  messages, isStreaming, error, insights, habits, mode = 'mentor', isKeyboardOpen = false, onStatusChange
}: StreamingChatProps) {
  const { get } = useConfig();
  ...
  const { data: status } = useQuery({
    ...
    queryFn: async () => { const res = await fetch('/api/user/status'); return res.json(); },
  ...
  const showHeartbeat = isStreaming && (lastMessage?.role === 'user' || rawContent === '' || (currentPage === 0 && wordIndex === 0));
